...
int main(int argc, char** argv){
  ...
  while(ros::ok())
  {
	...
	//cout << elapsed<<","<<pos_error<<",";
  	...
  	outputFile<<endl;
	pos++;
	if(pos == 1) calculateGoal(goal,-0.9415,-1.517,0); // x, y , quaternion z
	else if (pos == 2) calculateGoal(goal,-2.0205,-2.8298,0);
	else if (pos == 3) calculateGoal(goal,-3.2077,-4.3628,0);
    ...
    outputFile<<"Time,PosError,";
  for (int i=0; i<rss_goal.rss.size(); i++) outputFile<<rss_goal.rss[i].name.c_str()<<","; //each AP
  outputFile.close();
  return 0;
}
